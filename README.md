...
### Neural model menu (starter scaffolding)
The worker includes a lightweight model registry so you can plug in neural network backends later without
changing the orchestration code. Initial options are listed below and can be extended in research mode:
...
### Neural training CLI
Install worker ML dependencies from `apps/worker/requirements.txt`, then run:
```bash
cd apps/worker
pip install -r requirements.txt
python -m cli train-model \\
  --data-path path/to/features.csv \\
  --feature-cols close,volume,atr,trend \\
  --target-col target \\
  --sequence-length 20 \\
  --model-name temporal_transformer \\
  --epochs 5 \\
  --output-path trained_model.pt
```
